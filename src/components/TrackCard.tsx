...
const TrackCard = ({ track, variant = 'grid' }: TrackCardProps) => {
  const { playTrack, toggleFavorite, currentTrack, isPlaying, togglePlay } = useMusic();
  ...
  if (variant === 'list') {
    return (
      ...
      >
        ...
        >
          <Heart
            className={`w-5 h-5 ${track.isFavorite ? 'fill-primary text-primary' : ''}`}
          />
        </button>
      </div>
    );
  ...
  return (
    <div className="group flex-shrink-0 w-36">
      <div className="relative aspect-square rounded-xl overflow-hidden mb-2">
        ...
        <button
          onClick={handlePlay}
          className="absolute bottom-2 right-2 w-10 h-10 rounded-full bg-primary flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-y-2 group-hover:translate-y-0"
        >
          {isCurrentTrack && isPlaying ? (
